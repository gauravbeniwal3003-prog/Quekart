...
export const INDIA_STATES_AND_DISTRICTS: StateLocationData[] = [
  {
  ...
  },
  {
    state: "Maharashtra",
    districts: [
      "Ahmednagar", "Akola", "Amravati", "Chhatrapati Sambhaji Nagar (Aurangabad)", 
  ...
  },
  {
    ...
    districts: [
      ...
      "Patiala", "Rupnagar", "Sahibzada Ajit Singh Nagar (Mohali)", "Sangrur", 
  ...
  },
  {
    ...
    districts: [
      ...
      "Karauli", "Kekri", "Kota", "Kotputli-Behror", "Khairthal-Tijara", "Nagaur", 
...
// Helper to get all sorted state names
export function getAllIndianStates(): string[] {
  return INDIA_STATES_AND_DISTRICTS.map(s => s.state).sort((a, b) => a.localeCompare(b));
}

// Helper to get sorted districts for a given state name
export function getDistrictsForState(stateName: string): string[] {
  ...
  if (found) {
    return [...found.districts].sort((a, b) => a.localeCompare(b));
  }
  return [];
}

// Find matching state name loosely
export function findMatchingState(input: string): string | null {
  if (!input) return null;
  const clean = input.toLowerCase().trim();
  const match = INDIA_STATES_AND_DISTRICTS.find(s => 
    s.state.toLowerCase() === clean ||
    clean.includes(s.state.toLowerCase()) ||
    s.state.toLowerCase().includes(clean)
  );
  return match ? match.state : null;
}

// Find matching district name loosely for a state
export function findMatchingDistrict(stateName: string, districtInput: string): string | null {
  ...
  if (districts.length === 0 || !districtInput) return null;
  const clean = districtInput.toLowerCase().trim();
